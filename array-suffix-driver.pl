...
array-suffix-driver.pl takes as input one or more text files 
...
if ( defined $opt_frequency ) { $sarray->set_frequency($opt_frequency);    }
else                          { $sarray->set_frequency(0);                 }

#  check the remove option
if ( defined $opt_remove )    { $sarray->set_remove($opt_remove);          }
else                          { $sarray->set_remove(0);                    }
                         
#  check to see if the marginals are wanted
if ( defined $opt_marginals ) { $sarray->set_marginals(1);                 }

#  check to see if the token option has been defined
if ( defined $opt_token )     { $sarray->set_token_file($opt_token);       }

#  check to see if the nontoken option has been defined
if ( defined $opt_nontoken )  { $sarray->set_nontoken_file($opt_nontoken); }

#  check to see if the stop option has been defined
if ( defined $opt_stop )      { $sarray->create_stop_list($opt_stop);      }

#  check to see if the newline option has been defined
if ( defined $opt_newline )   { $sarray->set_new_line();                   }
...
if ( defined $opt_stopmode ) { 
    ...
    $sarray->set_stop_mode($opt_stopmode);    
}  
else { $sarray->set_stop_mode(OR); }
...
if ( defined $opt_ngram )     { 
    ...
    $sarray->set_ngram_size($opt_ngram); 
}
else { $sarray->set_ngram_size(2); }
...
$sarray->set_destination_file($destination);
...
sub askHelp
{
    print STDERR "Type array-suffix-driver.pl --help for help.\n";
}
...
{
    print STDERR "Usage: array-suffix-driver.pl [OPTIONS] DESTINATION SOURCE [[, SOURCE] ...]\n";
    askHelp();
}
...
sub showHelp
{
    print "Usage: array-suffix-driver.pl [OPTIONS] DESTINATION SOURCE [[, SOURCE] ...]\n\n";
